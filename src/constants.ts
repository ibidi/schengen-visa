...
export const SCHENGEN_ULKELERI = [
...
export const getCountryConfig = (countryId: string): CountryConfig | undefined => {
  return COUNTRY_CONFIGS.find(c => c.id === countryId.toLowerCase());
};

/**
 * Ülke adına göre config getir
 */
export const getCountryConfigByName = (countryName: string): CountryConfig | undefined => {
  return COUNTRY_CONFIGS.find(c => c.name.toLowerCase() === countryName.toLowerCase());
};

export const VIZE_MERKEZLERI: Record<string, VizeMerkezi> = {
  fransa: {
